...
class TestSampleAssociate(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.base_create_sample_process = \
            self.base_experiment.create_process_from_template(Template.create)
        self.base_sample = self.base_create_sample_process.create_samples(['Test Sample 1'])[0]
        self.base_compute_process = \
            self.base_experiment.create_process_from_template(Template.compute)
    ...
    def test_associate_sample_with_process(self):
        process_with_sample = \
            self.base_compute_process.add_samples_to_process([self.base_sample])
        ...
        for process in process_with_sample.input_samples[0].processes:
            if (process.process_type == 'analysis'): found_process = process
