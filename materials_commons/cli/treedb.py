...
class TreeTable(SqlTable):
    ...
    def __init__(self, proj):
        ...
        if not hasattr(proj, 'local_path') or proj.local_path is None:
            raise clifuncs.MCCLIException("Error constructing TreeTable: no 'local_path' attribute in 'proj'. Use `materials_commons.cli.functions.make_local_project` to construct 'proj'.")
        self.proj = proj
    ...
    def _walk_results(self, path):
        ...
        if not len(results) == 1:
            raise MCCLIException("TreeTable.walk error: multiple records for path '" + path + "'")

        top = results[0]
        if not top['otype'] == 'directory':
            raise MCCLIException("TreeTable.walk error: '" + path + "' is not a directory")
...
class RemoteTree(TreeTable):
    ...
    def _make_record(self, file_or_dir, checktime):
        ...
        for key in self.tablecolumns():
            if key == "id":
                ...
                record["parent_id"] = clifuncs.get_parent_id(file_or_dir)
            elif key == "checktime":
                record["checktime"] = checktime
            elif key == "mtime":
                record["mtime"] = clifuncs.epoch_time(file_or_dir.input_data.get("mtime", None))
            elif key == "parent_path":
                parent_path = os.path.dirname(file_or_dir.input_data["path"])
                if not parent_path:
                    parent_path = None
                record["parent_path"] = parent_path
            elif key not in file_or_dir.input_data:
                continue
            elif file_or_dir.input_data[key] is None:
                continue
            elif isinstance(file_or_dir.input_data[key], str) and not file_or_dir.input_data[key]:
                continue
            else:
                record[key] = file_or_dir.input_data[key]
        return record
    ...
    def _check(self, proj, path, checktime=None, get_children=True):
        ...
        if get_children:
            children = []
            if file_or_dir_obj is not None and isinstance(file_or_dir_obj, mcapi.Directory):
                for child in clifuncs.get_children(file_or_dir_obj):
                    _checktime = checktime
                    if isinstance(child, mcapi.Directory):
                        _checktime = None
