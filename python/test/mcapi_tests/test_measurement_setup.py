...
class TestMeasurementSetup(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.property_set_id = cls.sample.property_set_id
        cls.process = cls.process.add_samples_to_process(cls.samples)
    ...
    def test_is_setup_correctly(self):
        ...
        table = self.process.make_table_of_samples_with_property_set_ids(self.samples)
        print "---- table of samples with property set ids"
        self.assertTrue(len(table), 1)
        self.assertIsNotNone(table[0]['property_set_id'])
        self.assertIsNotNone(table[0]['sample'])
        self.assertIsNotNone(table[0]['sample'].id)
        self.assertIsNotNone(table[0]['sample'].name)
        self.assertEqual(table[0]['sample'].id, self.sample.id)
        self.assertEqual(table[0]['sample'].name, self.sample.name)
